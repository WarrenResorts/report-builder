...
export class CSVParser extends BaseFileParser {
  ...
  private looksLikeCSV(buffer: Buffer): boolean {
    ...
    for (const delimiter of delimiters) {
      ...
      if (firstLineCount > 0 && Math.abs(firstLineCount - secondLineCount) <= 1) {
        return true;
      }
    }
  ...
  private async parseCSVContent(
    ...
    const rawRows = this.parseCSVRows(content, delimiter, options.quote || '"', options.skipEmptyLines || false);
    ...
    if (options.hasHeaders && rawRows.length > 0) {
      ...
      if (this.hasNonStringHeaders(headers)) {
        warnings.push('Some headers appear to be numeric - may not be actual headers');
      }
    }
    
    // Convert to objects if headers exist
    const rows = headers 
      ? dataRows.map(row => this.rowToObject(row, headers!, statistics.maxColumns, warnings))
      : dataRows;
    
    // Add warnings for data quality issues
    if (statistics.inconsistentColumnCounts > 0) {
      warnings.push(`${statistics.inconsistentColumnCounts} rows have inconsistent column counts`);
    }
    
    if (dataRows.length > 10000) {
      warnings.push(`Large CSV file with ${dataRows.length} rows - consider processing in chunks`);
    }
  ...
  private detectDelimiter(content: string, warnings: string[]): string {
    ...
    if (bestDelimiter !== ',') {
      warnings.push(`Detected delimiter: '${bestDelimiter === '\t' ? '\\t' : bestDelimiter}'`);
    }
   ...
   */
  private parseCSVRows(content: string, delimiter: string, quote: string, skipEmptyLines: boolean): string[][] {
    const rows: string[][] = [];
    ...
    while (i < content.length) {
      ...
      if (char === quote) {
        ...
        if (!skipEmptyLines || currentRow.length > 1 || (currentRow.length === 1 && currentRow[0].trim().length > 0)) {
  ...
  private rowToObject(
    ...
    for (let i = 0; i < Math.max(row.length, headers.length); i++) {
      ...
      if (i >= headers.length && row[i]) {
        warnings.push(`Row has more columns than headers, using generated header: ${header}`);
      }
  ...
  private hasNonStringHeaders(headers: string[]): boolean {
    return headers.some(header => !isNaN(Number(header)) && header.trim() !== '');
  }
