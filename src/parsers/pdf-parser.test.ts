...
describe('PDFParser', () => {
  ...
  describe('parseFromBuffer', () => {
    ...
    it('should include raw content when requested', async () => {
      ...
      const result = await parser.parseFromBuffer(pdfBuffer, 'test.pdf', config);
      
      expect(result.success).toBe(true);
      expect((result.data as any).rawContent).toBeDefined();
      expect((result.data as any).rawContent).toBe(pdfBuffer.toString('base64'));
    });
    ...
    it('should add warning for page limit reached', async () => {
      ...
      const result = await parser.parseFromBuffer(pdfBuffer, 'large.pdf', config);
      
      Math.random = originalRandom;
      
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('more than'))).toBe(true);
    });
    ...
    it('should add warning for low text content', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('very little text'))).toBe(true);
    });
    ...
    it('should handle corrupted file errors', async () => {
      ...
      (parser as any).extractPDFContent = vi.fn().mockRejectedValue(new Error('File is corrupted and cannot be processed'));
    ...
    it('should include additional metadata', async () => {
      ...
      expect(result.metadata.additionalMetadata).toHaveProperty('averageTextPerPage');
    });
  });
  ...
  describe('error code determination', () => {
    it('should determine timeout error code', async () => {
      ...
      const originalExtract = (parser as any).extractPDFContent;
      (parser as any).extractPDFContent = vi.fn().mockRejectedValue(new Error('Operation timed out after 1000ms'));
    ...
    it('should determine file size error code', async () => {
      ...
      const originalExtract = (parser as any).extractPDFContent;
      (parser as any).extractPDFContent = vi.fn().mockRejectedValue(new Error('File size exceeds maximum allowed'));
    ...
    it('should determine damaged file error code', async () => {
      ...
      const originalExtract = (parser as any).extractPDFContent;
      (parser as any).extractPDFContent = vi.fn().mockRejectedValue(new Error('File is damaged and cannot be read'));
    ...
    it('should default to parsing error code', async () => {
      ...
      const originalExtract = (parser as any).extractPDFContent;
      (parser as any).extractPDFContent = vi.fn().mockRejectedValue(new Error('Generic parsing error'));
