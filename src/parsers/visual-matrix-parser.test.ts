...
describe('VisualMatrixParser', () => {
  ...
  describe('Excel File Parsing', () => {
    it('should parse valid VisualMatrix Excel file', async () => {
      ...
      worksheet.addRow([
        2, '2001', 'Food Sales', 'FOOD-SALES', 201,
        0, '', '4020', '', 'Food & Beverage Revenue',
        1, new Date('2024-01-01'), new Date('2024-01-01')
      ]);

      const buffer = await workbook.xlsx.writeBuffer();

      const result = await parser.parseFromBuffer(Buffer.from(buffer), 'test.xlsx');
    ...
    it('should handle empty Excel file', async () => {
      ...
      expect(result.metadata.warnings).toContain('No data rows found in VisualMatrix sheet');
    });
    ...
    it('should filter out invalid rows', async () => {
      ...
      worksheet.addRow([
        3, '3001', 'Tax Revenue', 'TAX-REV', 301,
        0, '', '4030', '', 'Tax Revenue',
        1, new Date('2024-01-01'), new Date('2024-01-01')
      ]);
      ...
      expect(result.metadata.warnings).toContain('Skipped 1 invalid rows (missing required fields)');
    });
  });
  ...
  describe('Configuration Options', () => {
    it('should use custom sheet name', async () => {
      ...
      const result = await customParser.parseFromBuffer(Buffer.from(buffer), 'test.xlsx');
    ...
    it('should filter by property ID when configured', async () => {
      ...
      worksheet.addRow([
        1, '1001', 'Room Revenue', 'ROOM-REV', 101,
        123, 'Test Property', '4010', '', 'Room Revenue',
        1, new Date('2024-01-01'), new Date('2024-01-01')
      ]);
      ...
      worksheet.addRow([
        2, '2001', 'Food Sales', 'FOOD-SALES', 201,
        456, 'Other Property', '4020', '', 'Food Revenue',
        1, new Date('2024-01-01'), new Date('2024-01-01')
      ]);
      ...
      const result = await propertyParser.parseFromBuffer(Buffer.from(buffer), 'test.xlsx');
    ...
    it('should exclude empty mappings when configured', async () => {
      const strictParser = new VisualMatrixParser({ includeEmptyMappings: false });
      ...
      const result = await strictParser.parseFromBuffer(Buffer.from(buffer), 'test.xlsx');
  ...
  describe('Error Handling', () => {
    it('should handle invalid Excel file', async () => {
      ...
      expect(result.error?.message).toContain('Failed to parse VisualMatrix file');
    });
  ...
  describe('Data Validation', () => {
    ...
    it('should handle date parsing errors gracefully', async () => {
      ...
      worksheet.addRow([
        1, '1001', 'Room Revenue', 'ROOM-REV', 101,
        0, '', '4010', '', 'Room Revenue',
        1, 'invalid-date', 'another-invalid-date'
      ]);
  ...
  describe('Metadata Generation', () => {
    it('should generate accurate metadata', async () => {
      ...
      worksheet.addRow([
        2, '1001', 'Room Revenue Duplicate', 'ROOM-REV-2', 102,
        123, 'Test Property', '4010', '', 'Room Revenue',
        1, new Date('2024-01-01'), new Date('2024-01-01')
      ]);

      worksheet.addRow([
        3, '2001', 'Food Sales', 'FOOD-SALES', 201,
        0, '', '4020', '', 'Food Revenue',
        1, new Date('2024-01-01'), new Date('2024-01-01')
      ]);
