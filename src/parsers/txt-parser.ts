...
export class TXTParser extends BaseFileParser {
  ...
  private readonly patterns = {
    ...
    currency: /\$[\d,]+\.?\d*/,
    keyValue: /^[^:]+:\s*.+$/,
    address: /\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Lane|Ln|Court|Ct|Place|Pl)(?:\s|$)/i,
  };
  ...
  async parseFromBuffer(
    ...
    try {
      ...
      const { text, encoding } = this.detectEncodingAndConvert(fileBuffer, warnings);
  ...
  private detectEncodingAndConvert(
    ...
    if (ratio > 0.3) {
      ...
      confidence = 0.5;
      warnings.push('High ratio of non-ASCII characters detected - encoding may not be UTF-8');
    }
    ...
    try {
      ...
      if (text.includes('\uFFFD')) {
        confidence = 0.3;
        warnings.push('Replacement characters found - file may not be UTF-8 encoded');
      }
      
      return { text, encoding: { detected, confidence } };
    } catch (error) {
      throw new Error(`Failed to decode text file: ${(error as Error).message}`);
    }
  }
  ...
  private async parseTextContent(
    ...
    if (lines.length > processedLines.length) {
      warnings.push(`File has ${lines.length} lines, only processing first ${options.maxLines}`);
    }
  ...
  private calculateStatistics(lines: string[]) {
    ...
    return {
      ...
      averageLineLength: lineLengths.length > 0 ? Math.round(lineLengths.reduce((a, b) => a + b, 0) / lineLengths.length) : 0,
    };
  }
  ...
  private analyzeStructure(lines: string[], warnings: string[]) {
    ...
    const keyValueCount = nonEmptyLines.filter(line => this.patterns.keyValue.test(line)).length;
    ...
    const listCount = nonEmptyLines.filter(line => 
    ...
    const headerCount = nonEmptyLines.filter(line => 
      /^[A-Z\s]+:?\s*$/.test(line) || line.trim().toUpperCase() === line.trim()
    ).length;
   ...
   */
  private extractKeyValuePairs(lines: string[], warnings: string[]): Record<string, string> {
   ...
   */
  private extractTabularData(lines: string[], delimiter: string, warnings: string[]): string[][] {
    ...
    const inconsistentRows = columnCounts.filter(count => count !== mostCommonCount).length;
    
    if (inconsistentRows > 0) {
      warnings.push(`${inconsistentRows} rows have inconsistent column counts in tabular data`);
    }
  ...
  private findMostCommonValue(arr: number[]): number {
    ...
    return Number(Object.entries(counts).reduce((a, b) => counts[Number(a[0])] > counts[Number(b[0])] ? a : b)[0]);
  }
