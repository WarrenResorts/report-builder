...
describe('CSVParser', () => {
  ...
  describe('parseFromBuffer', () => {
    ...
    it('should handle UTF-8 BOM', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.metadata.warnings).toContain('UTF-8 BOM detected and removed');
    });
    ...
    it('should handle inconsistent column counts', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('inconsistent'))).toBe(true);
    });

    it('should handle large files with warning', async () => {
      // Create a large CSV (simulate)
      const header = 'name,age,city\n';
      const rows = Array.from({ length: 15000 }, (_, i) => `Person${i},${20 + i},City${i}`).join('\n');
      ...
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('Large CSV'))).toBe(true);
    });
  ...
  describe('advanced CSV parsing', () => {
    it('should handle CSV with specific delimiter', async () => {
      const csvContent = 'name|age|city\nJohn|30|NYC\nJane|25|LA';
      const buffer = Buffer.from(csvContent);
      const config = { parserOptions: { delimiter: '|', autoDetectDelimiter: false } };
    ...
    it('should handle CSV with auto-detection fallback', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('Could not reliably detect'))).toBe(true);
    });
    ...
    it('should handle CSV with non-string headers detected', async () => {
      ...
      const result = await parser.parseFromBuffer(buffer, 'numeric-headers.csv');
      
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('headers appear to be numeric'))).toBe(true);
    });
    ...
    it('should handle CSV with extra columns in data rows', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('more columns than headers'))).toBe(true);
    });
    ...
    it('should handle UTF-16 CSV files', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('UTF-16'))).toBe(true);
    });
    ...
    it('should not skip empty lines when configured', async () => {
      ...
      const result = await parser.parseFromBuffer(buffer, 'with-empty.csv', config);
      
      expect(result.success).toBe(true);
      expect((result.data as any).rawRows.some((row: string[]) => row.length === 1 && row[0] === '')).toBe(true);
    });
    ...
    it('should handle timeout errors', async () => {
      ...
      (parser as any).parseCSVContent = () => new Promise(resolve => setTimeout(() => resolve({
        rows: [],
        headers: ['name', 'age'],
        rawRows: [['name', 'age'], ['John', '30']],
        dataRowCount: 1,
        columnCount: 2,
        delimiter: ',',
        statistics: { emptyRows: 0, inconsistentColumnCounts: 0, maxColumns: 2, minColumns: 2 }
      }), 200));
  ...
  describe('delimiter detection edge cases', () => {
    ...
    it('should warn about non-comma delimiter detection', async () => {
      ...
      expect(result.success).toBe(true);
      expect((result.data as any).delimiter).toBe('\t');
      expect(result.metadata.warnings.some(w => w.includes('Detected delimiter'))).toBe(true);
    });
  });
  ...
  describe('CSV line parsing edge cases', () => {
    ...
    it('should handle fields with only delimiters', async () => {
      ...
      const result = await parser.parseFromBuffer(buffer, 'only-delimiters.csv');
