...
describe('TXTParser', () => {
  ...
  describe('canParse', () => {
    ...
    it('should handle mixed content with high control character ratio', () => {
      const mixedBuffer = Buffer.from([0x41, 0x42, 0x00, 0x01, 0x02, 0x03, 0x43]); // ABC with control chars
  ...
  describe('parseFromBuffer', () => {
    ...
    it('should detect report structure', async () => {
      const reportContent = 'MONTHLY REPORT\n\nSales Summary\nRevenue: $50,000\n\nEXPENSES\nMarketing: $10,000\nOperations: $15,000';
    ...
    it('should warn about high non-ASCII character ratio', async () => {
      const textContent = 'Тест с русскими символами и другими не-ASCII символами';
      const buffer = Buffer.from(textContent);
      
      const result = await parser.parseFromBuffer(buffer, 'russian.txt');
      
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('non-ASCII'))).toBe(true);
    });
    ...
    it('should warn about replacement characters', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('Replacement characters'))).toBe(true);
    });
...
ID: 12345`;
    ...
    it('should handle large files with line limit', async () => {
      ...
      expect(result.metadata.warnings.some(w => w.includes('only processing first'))).toBe(true);
    });
    ...
    it('should handle key-value pairs with duplicate keys', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.metadata.warnings.some(w => w.includes('Duplicate key'))).toBe(true);
    });
    ...
    it('should disable structure detection when configured', async () => {
      ...
      const result = await parser.parseFromBuffer(buffer, 'no-structure.txt', config);
    ...
    it('should handle timeout', async () => {
      ...
      (parser as any).parseTextContent = () => new Promise(resolve => setTimeout(() => resolve({
        text: textContent,
        lines: [textContent],
        structureType: 'unstructured',
        statistics: { lineCount: 1, wordCount: 2, characterCount: 12, emptyLines: 0, longestLine: 12, averageLineLength: 12 },
        patterns: { hasTimestamps: false, hasEmailAddresses: false, hasPhoneNumbers: false, hasAddresses: false, hasCurrency: false, hasNumbers: false },
