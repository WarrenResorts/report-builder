...
export class PDFParser extends BaseFileParser {
      ...
      return {
        ...
        metadata: {
          ...
          additionalMetadata: {
            ...
            averageTextPerPage: Math.round(parsedData.text.length / parsedData.pageCount),
          },
        },
      };
  ...
  private async extractPDFContent(
    ...
    if (simulatedPageCount >= (options?.maxPages || 100)) {
      warnings.push(`PDF has more than ${options?.maxPages} pages, only first ${options?.maxPages} processed`);
    }
  ...
  private extractDocumentInfo(_buffer: Buffer) {
    ...
    return {
      ...
      creationDate: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000), // Last 30 days
   ...
   */
  private generateSimulatedPageText(pageNumber: number, title?: string): string {
