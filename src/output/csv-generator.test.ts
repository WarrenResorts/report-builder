...
import { CSVGenerator, generateCSV, generateMultipleCSVs, type CSVGeneratorConfig } from './csv-generator';
...
describe('CSV Generator Unit Tests', () => {
  ...
  describe('generateCSV', () => {
    it('should generate CSV with headers by default', async () => {
      const result = await generator.generateCSV(mockTransformedData, 'test-correlation-id');
      ...
      expect(lines[1]).toContain('true,30,john@example.com,"John Doe",75000.5,2023-01-01');
      expect(lines[2]).toContain('false,25,jane@example.com,"Jane Smith",65000,2023-02-01');
    });
    ...
    it('should generate CSV without headers when configured', async () => {
      ...
      expect(result.success).toBe(true);
      const lines = result.csvContent!.split('\n');
      expect(lines[0]).toContain('true,30,john@example.com,"John Doe",75000.5,2023-01-01');
    ...
    it('should use custom field ordering when provided', async () => {
      ...
      expect(lines[1]).toContain('"John Doe",john@example.com,30,true,75000.5,2023-01-01');
    });
    ...
    it('should sanitize field values when enabled', async () => {
      ...
      const result = await generator.generateCSV(dataWithSpecialChars, 'test-correlation-id');
    ...
    it('should truncate long field values', async () => {
      ...
      const result = await generator.generateCSV(dataWithLongField, 'test-correlation-id');
    ...
    it('should handle empty data gracefully', async () => {
      ...
      const result = await generator.generateCSV(emptyData, 'test-correlation-id');
    ...
    it('should handle complex data types', async () => {
      ...
      const result = await generator.generateCSV(complexData, 'test-correlation-id');
    ...
    it('should quote fields with special characters', async () => {
      ...
      const result = await generator.generateCSV(specialData, 'test-correlation-id');
      ...
      expect(result.csvContent).toContain('"Value with leading/trailing spaces"'); // Spaces quoted and trimmed
    });
    ...
    it('should provide accurate statistics', async () => {
      ...
      expect(result.stats).toEqual({
        totalRecords: 2,
        fieldCount: 6,
        includedFields: ['Active', 'Age', 'Email', 'Name', 'Salary', 'StartDate'],
        sanitizedValues: 0,
        truncatedValues: 0,
    ...
    it('should handle errors gracefully', async () => {
      ...
      invalidData.records[0].fields.CircularRef.self = invalidData.records[0].fields.CircularRef;

      const result = await generator.generateCSV(invalidData, 'test-correlation-id');
  ...
  describe('generateMultipleCSVs', () => {
    it('should generate CSVs for multiple transformed data files', async () => {
      ...
      const results = await generator.generateMultipleCSVs(multipleData, 'test-correlation-id');
  ...
  describe('convenience functions', () => {
    it('should work with generateCSV convenience function', async () => {
      const result = await generateCSV(mockTransformedData, 'test-correlation-id');
    ...
    it('should work with generateMultipleCSVs convenience function', async () => {
      ...
      const results = await generateMultipleCSVs(multipleData, 'test-correlation-id');
    ...
    it('should accept custom configuration in convenience functions', async () => {
      ...
      const result = await generateCSV(mockTransformedData, 'test-correlation-id', config);
  ...
  describe('field ordering edge cases', () => {
    it('should handle partial field ordering', async () => {
      ...
      expect(headers.slice(2)).toEqual(['Active', 'Age', 'Salary', 'StartDate']);
    });
