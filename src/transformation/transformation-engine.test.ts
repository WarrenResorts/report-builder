...
describe('TransformationEngine', () => {
  ...
  describe('transformData', () => {
    it('should successfully transform CSV data', async () => {
      const result = await engine.transformData(mockRawData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle PDF data transformation', async () => {
      ...
      const result = await engine.transformData(pdfRawData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle TXT data transformation', async () => {
      ...
      const result = await engine.transformData(txtRawData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle missing property mapping', async () => {
      ...
      await expect(
        engine.transformData(unmappedRawData, mockMappingData, 'test-correlation-id')
    ...
    it('should handle required field validation errors', async () => {
      ...
      await expect(
        engine.transformData(invalidRawData, mockMappingData, 'test-correlation-id')
    ...
    it('should continue on errors when configured', async () => {
      ...
      const engine = new TransformationEngine(config);
      const result = await engine.transformData(mixedRawData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle data type conversions', async () => {
      ...
      const result = await engine.transformData(typeTestData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle validation failures in lenient mode', async () => {
      ...
      expect(result.records).toHaveLength(2);
      expect(result.records[0].metadata.transformationWarnings.length).toBeGreaterThan(0);
    });
  });

  describe('convenience functions', () => {
    it('should work with transformFileData function', async () => {
      const result = await transformFileData(mockRawData, mockMappingData, 'test-correlation-id');
    ...
    it('should work with transformMultipleFiles function', async () => {
      ...
      const results = await transformMultipleFiles(multipleRawData, mockMappingData, 'test-correlation-id');
  ...
  describe('nested field extraction', () => {
    it('should extract nested field values', async () => {
      ...
      const result = await engine.transformData(nestedRawData, mockMappingData, 'test-correlation-id');
  ...
  describe('error handling', () => {
    it('should handle type conversion errors', async () => {
      ...
      const engine = new TransformationEngine(config);
      const result = await engine.transformData(invalidTypeData, mockMappingData, 'test-correlation-id');
    ...
    it('should respect max errors limit', async () => {
      ...
      const engine = new TransformationEngine(config);
      const result = await engine.transformData(manyInvalidRecords, mockMappingData, 'test-correlation-id');

      expect(result.metadata.errors.length).toBeLessThanOrEqual(5);
      expect(result.metadata.warnings).toContain('Stopped processing after 5 errors');
    });
  });
  ...
  describe('edge cases and additional coverage', () => {
    it('should handle empty content arrays', async () => {
      ...
      const result = await engine.transformData(emptyRawData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle content with non-array structure for CSV', async () => {
      ...
      const result = await engine.transformData(invalidStructureData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle TXT content with structured data array', async () => {
      ...
      const result = await engine.transformData(txtWithStructuredData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle unknown file types', async () => {
      ...
      const result = await engine.transformData(unknownTypeData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle transformation with undefined nested field', async () => {
      ...
      const result = await engine.transformData(nestedData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle boolean conversion from numbers', async () => {
      ...
      const result = await engine.transformData(booleanTestData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle validation with allowedValues that fails', async () => {
      ...
      const engine = new TransformationEngine(config);
      const result = await engine.transformData(validationTestData, mockMappingData, 'test-correlation-id');

      expect(result.records[0].metadata.transformationWarnings.length).toBeGreaterThan(0);
      expect(result.records[0].metadata.transformationWarnings[0]).toContain('Value not in allowed list');
    });
    ...
    it('should handle validation with minLength failure', async () => {
      ...
      expect(result.records[0].metadata.transformationWarnings.length).toBeGreaterThan(0);
      expect(result.records[0].metadata.transformationWarnings[0]).toContain('Value too short');
    });
    ...
    it('should handle validation with maxLength failure', async () => {
      ...
      expect(result.records[0].metadata.transformationWarnings.length).toBeGreaterThan(0);
      expect(result.records[0].metadata.transformationWarnings[0]).toContain('Value too long');
    });
    ...
    it('should handle unknown data types (default case)', async () => {
      ...
      expect(result.records[0].fields.CustomField).toEqual({ complex: 'object' });
    });
    ...
    it('should handle unknown transformation types (default case)', async () => {
      ...
      const result = await engine.transformData(transformationTestData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle date values that are already Date objects', async () => {
      ...
      const result = await engine.transformData(dateTestData, mockMappingData, 'test-correlation-id');
    ...
    it('should handle invalid date conversion errors', async () => {
      ...
      const engine = new TransformationEngine(config);
      const result = await engine.transformData(invalidDateData, mockMappingData, 'test-correlation-id');

      expect(result.records[0].metadata.transformationWarnings.length).toBeGreaterThan(0);
      expect(result.records[0].metadata.transformationWarnings[0]).toContain('Cannot convert');
    });
  });
  ...
  describe('index exports', () => {
    ...
    it('should export convenience functions', () => {
      ...
      expect(typeof TransformationModule.transformMultipleFiles).toBe('function');
    });
  });
