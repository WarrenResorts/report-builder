...
### Phase 12: IHG / Opera daily files (new pipeline) ✅ **[COMPLETED — PRODUCTION LIVE]**

IHG-type properties send **Opera text exports** (`trial_balance*.txt`, `stat_dmy_seg*.txt`) instead of a Visual Matrix PDF. Outputs are the same **JE** and **StatJE** CSVs delivered via the same email/S3 pipeline as Visual Matrix properties. Technical reference in **`docs/opera-ihg-pipeline.md`**.

**First property live:** `holiday-inn-express-clover-lane` (sender: `hsaor-donotreply@hotels.ihg.com`), deployed to production June 2026.

- [x] Email processor accepts Opera `.txt` attachment types
- [x] Sender → slug mapping via SSM `email-mapping` for Opera properties
- [x] Opera file detection by filename pattern (`trial_balance*`, `stat_dmy_seg*`) in `FileProcessor`
- [x] Opera path branched correctly — VM mapping and VM-only steps are skipped
- [x] `parseTrialBalance` parser: CSV rows, `TRX_CODE` / `DESCRIPTION` / ledger columns / `TRX_DATE`, summary block (`CS_` prefixed keys) parsed into `summaryEntries` map
- [x] `parseStatDmySeg` parser: header + segment rows + summary rows; business date extracted
- [x] Trial balance + stat file pairing by `propertyId` + processing date
- [x] `OperaMappingLoader`: loads latest XLSX from `opera/` S3 prefix; supports **dual mapping** (`Map<string, OperaMappingEntry[]>`) so one `TRX_CODE` can map to multiple GL accounts
- [x] `OperaTransformation`: builds JE lines from transaction rows and summary block entries; summary-block-driven accounts (AR City Ledger, Deferred Revenue, Guest Ledger) use `CS_` keys for reliable netting
- [x] ADR, Occupancy, and RevPAR calculated from `roomsAvailable` (property config) + `totalRoomsOccupied` (stat data) + `ROOM_REVENUE_TRX_CODES`
- [x] Visa / MasterCard / Discover combined into single `Visa/MC/Discover` JE line (mirrors Visual Matrix behavior)
- [x] `property-config.ts` updated: `holiday-inn-express-clover-lane` with correct `subsidiaryFullName`, `roomsAvailable: 65`
...
### Phase 13: Dependency Maintenance 🔧 **[ONGOING]**
- [x] `npm audit` threshold raised to `high` only — eliminates recurring churn from moderate advisories in dev tooling (brace-expansion, postcss) that keep getting re-issued with new IDs
- [x] All Dependabot version bumps consolidated into single PR (#174) — all non-breaking packages updated to latest
- [x] `aws-actions/configure-aws-credentials` updated to v6.2.0 in CI workflow
- [ ] **`pdf-parse` v2 migration** — Blocked. v2 replaced the function-based API with a class-based API and removed the `pagerender` callback that we use to insert pipe delimiters for columnar text parsing in Visual Matrix PDFs. `pdfjs-dist` was previously investigated as a direct replacement but could not be made to work. Needs dedicated investigation before proceeding. Current v1 is stable and functional.
...
4. **`feature/ihg-opera-pipeline`** ← **COMPLETED ✅** (merged to main, v0.9.0)
   - Opera trial balance + stat parsers
   - Opera XLSX mapping loader with dual-mapping support
   - FileProcessor routing for Opera vs Visual Matrix
   - ADR / Occupancy / RevPAR stat calculations
   - Visa/MC/Discover combined JE line
   - CI/CD deployment guard improvements

5. **`feat/day-to-day-comparison`** ← **FUTURE BRANCH**
...
6. **`feat/comprehensive-monitoring`** ← **FUTURE BRANCH**
...
5. **✅ COMPLETED**: Phase 6 - Duplicate Detection & Reprocessing Override
