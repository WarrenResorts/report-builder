...
### Phase 12: IHG / Opera daily files (new pipeline) 🔄 **[IN PROGRESS]**

IHG-type properties send **Opera text exports** (`trial_balance*.txt`, `stat_dmy_seg*.txt`) instead of a Visual Matrix PDF. Outputs must remain the same **JE** and **StatJE** CSVs and email/S3 delivery as today. Technical reference (samples, mapping columns, storage rules) is in **`docs/opera-ihg-pipeline.md`**. Mapping workbook format: **`WARRENRESORTS_Opera_2025_02_13_18_40_01.xlsx`** (sheet `Opera`, `TRX_CODE` → NetSuite `Glacct Code` / `Glacct Suffix`).

**Property identity:** NetSuite config and duplicate markers must use the **email-mapping slug** (S3 path `daily-files/{slug}/`), not PDF-extracted names or opaque attachment suffixes.

**Mapping storage:** Opera mapping must **not** share the “newest file in entire mapping bucket” rule with Visual Matrix—use a reserved S3 prefix (e.g. `opera/`) or filename convention so only the correct workbook is selected for Opera runs.

#### Step checklist (implementation order)

1. **Inbound**
   - [ ] Confirm email processor allows Opera attachment types (`.txt`, any others IHG uses); extend validation if needed.
   - [ ] Confirm sender → slug via existing SSM `email-mapping`; add entries per Opera property when onboarding.

2. **Detection and routing**
   - [ ] Define reliable detection for Opera inbound (filename patterns, content sniffing, or metadata) in `FileProcessor` / parser factory.
   - [ ] Branch Opera path: skip `applyVisualMatrixMappings` and VM-only steps that do not apply; do not load VM mapping for Opera batches.

3. **Parsers**
   - [ ] Parser for `trial_balance*.txt`: CSV rows, `TRX_CODE` / `DESCRIPTION` / ledger columns / `TRX_DATE`, ignore or parse footer sections after the main transaction block.
   - [ ] Parser for `stat_dmy_seg*.txt`: header + segment rows + summary rows; extract business date.

4. **Pairing and dates**
   - [ ] Rule for matching trial balance + stat file for same `propertyId` + processing date (both required vs partial).
   - [ ] Single canonical **business date** per run (from `TRX_DATE`, stat header, or agreed fallback).

5. **Opera mapping**
   - [ ] `loadOperaMapping` (or equivalent): list latest file under Opera prefix, parse sheet `Opera` with the 22-column schema documented in `docs/opera-ihg-pipeline.md`.
   - [ ] Join trial balance lines to mapping by `TRX_CODE` (and optional `DESCRIPTION` ↔ `Descr`).

6. **NetSuite output**
   - [ ] Build JE lines into existing `JournalEntryGenerator` inputs (debit/credit, subsidiary, location from **slug-resolved** `property-config`).
   - [ ] Build StatJE via existing `StatisticalEntryGenerator`; add **stat-specific mapping** if segment rows are not covered by the Opera TRX workbook (second sheet or second file—TBD with finance).

7. **Property config**
   - [ ] Implement slug-based resolution (Option A: `propertyName` = email slug for Opera rows, or Option B: explicit `emailSlug` / `inboundPropertyId` field—see onboarding doc).
   - [ ] Unit tests for Opera property config lookup and duplicate key consistency.

8. **Integration and ops**
   - [ ] End-to-end test: sample `trial_balance` + `stat_dmy_seg` + mapping XLSX → JE + StatJE matching reference CSVs.
   - [ ] Update `docs/opera-ihg-pipeline.md` (and this plan) with final S3 prefix, any new SSM keys, and SES checklist per property.
   - [ ] `npm run build`, `lint`, `format:check`, `npm test` before merge.
...
- **Visual Matrix / PDF pipeline**: Phases 3–6, 8 (partial), 10 complete; production in use for existing properties.
- **Active development**: **Phase 12 — IHG / Opera** (feature branch such as `feature/ihg-property-types-process`): new parsers, Opera mapping load path, slug-based property config, `FileProcessor` routing—see Phase 12 checklist above and **`docs/opera-ihg-pipeline.md`**.
- **Next (parallel or after Phase 12 MVP)**: Phase 7 (day-to-day comparison), remaining Phase 8/9 items.
...
3. **`feat/duplicate-detection`** ← **COMPLETED ✅** (merged to main)
...
6. **`feature/ihg-property-types-process`** (or equivalent) ← **IHG / OPERA**
   - Phase 12 checklist: parsers, Opera XLSX mapping, `FileProcessor` branch, slug-based config, StatJE mapping gap if any
   - Keeps Visual Matrix behavior unchanged for existing properties
...
6. **🚀 IN PROGRESS: Phase 12 - IHG / Opera daily files**
   - Execute the Phase 12 step checklist (inbound → parsers → Opera mapping → JE/StatJE → slug-based config → tests → docs)
   - Reference: **`docs/opera-ihg-pipeline.md`** (mapping column spec + engineering notes)
7. **NEXT (after Phase 12 MVP or in parallel): Phase 7 - Day-to-Day Comparison Engine**
