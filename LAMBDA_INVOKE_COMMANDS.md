...
**Reprocess by business date (filters files by actual business date in PDF, not received date):**
```bash
export AWS_PROFILE=dev-account && aws lambda invoke --function-name report-builder-file-processor-development --cli-binary-format raw-in-base64-out --payload '{"source":"aws.events","detail-type":"Scheduled Event","detail":{"processingType":"daily-batch","businessDate":"2026-01-12"}}' /tmp/lambda-response.json && cat /tmp/lambda-response.json | jq .
```
> Replace `2026-01-12` with the business date you want to reprocess. This queries multiple folder dates and filters to only files matching the specified business date inside the PDF. Use this for reprocessing after bug fixes.
